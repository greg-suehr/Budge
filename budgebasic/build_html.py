...
def wrap_table_row(row, css_class, id):
    """helper method used in transaction_row, ..."""
    return '<tr class="table-row %s" id="%s-%s">' % (css_class, css_class, id) + row + '</tr>'
...
with open("../template.html", "r") as f:
    html = "".join([_.strip() for _ in f.readlines()])
...
for transaction in transactions:
    transaction_html += transaction_row(transaction)
html = html.replace("#anchor", transaction_html)
